...
class TeacherTrajBatch(NamedTuple):
    """Teacher rollout with leading time dimension (T, N, ...)."""
...
def init_teacher_rollout_buffer(buffer_size, num_envs, teacher_obs_dim, dtype):
    return TeacherRolloutBuffer(
        ...
        done=jnp.zeros((buffer_size, num_envs), dtype=dtype),
...
def push_teacher_transition(buffer, transition):
    ...
    return TeacherRolloutBuffer(
        ...
        done=buffer.done.at[idx].set(transition.done),
...
def push_teacher_rollout_on_done(
    buffer, carry, teacher_reward, done, teacher_ep_step, teacher_episode_length
):
    """Push a teacher transition when any env is done; update episode step/done."""

    def _do_push(operands):
        b, ep_step = operands
        new_ep_step = ep_step + 1
        done_flag = (
            new_ep_step >= teacher_episode_length
        ).astype(teacher_reward.dtype)
        done_row = jnp.broadcast_to(done_flag, teacher_reward.shape)
        new_ep_step = jnp.where(
            new_ep_step >= teacher_episode_length,
            jnp.array(0, dtype=ep_step.dtype),
            new_ep_step,
        )
        ...
        return push_teacher_transition(b, transition), new_ep_step

    def _skip_push(operands):
        return operands
...
def gather_teacher_traj(buffer, buffer_size):
    """Gather ordered ring-buffer slots as a (T, N, ...) teacher traj batch."""
    start = (buffer.write_idx - buffer_size) % buffer_size
    slot_indices = (start + jnp.arange(buffer_size)) % buffer_size

    def _gather(field):
        return field[slot_indices]

    return TeacherTrajBatch(
        obs=_gather(buffer.teacher_obs),
        action=_gather(buffer.goal_idx),
        value=_gather(buffer.value),
        reward=_gather(buffer.reward),
        log_prob=_gather(buffer.log_prob),
        done=_gather(buffer.done),
    )


def flatten_teacher_traj(traj):
    """Flatten a (T, N, ...) teacher traj into a PPO minibatch batch."""
    num_envs = traj.obs.shape[1]
    buffer_size = traj.obs.shape[0]

    def _flatten(field):
        return field.reshape((buffer_size * num_envs,) + field.shape[2:])

    return TeacherFlatBatch(
        obs=_flatten(traj.obs),
        action=_flatten(traj.action),
        value=_flatten(traj.value),
        reward=_flatten(traj.reward),
        log_prob=_flatten(traj.log_prob),
        done=_flatten(traj.done),
    )


def flatten_teacher_rollout_buffer(buffer, buffer_size):
    """Flatten valid ring-buffer slots into a teacher PPO batch."""
    return flatten_teacher_traj(gather_teacher_traj(buffer, buffer_size))
...
def reset_teacher_rollout_buffer(buffer):
    return TeacherRolloutBuffer(
        ...
        done=jnp.zeros_like(buffer.done),
        write_idx=jnp.array(0, dtype=jnp.int32),
        count=jnp.array(0, dtype=jnp.int32),
    )
...
def make_train(config):
    ...
    assert int(config["TEACHER_EPISODE_LENGTH"]) >= 1, (
        "TEACHER_EPISODE_LENGTH must be >= 1"
    )
    assert (
        teacher_batch_size
    ...
    teacher_episode_length = int(config["TEACHER_EPISODE_LENGTH"])
    ...
    def train(rng):
        ...
        teacher_ep_step = jnp.array(0, dtype=jnp.int32)
        goals = raw_goals
        if condition_on_goal:
            if config["NORMALIZE_ENV"]:
        ...
        def _update_step(runner_state, update_idx):
            ...
            def _env_step(runner_state, unused):
                *body, rng = runner_state
                ...
                teacher_rollout_buffer, teacher_ep_step = push_teacher_rollout_on_done(
                    teacher_rollout_buffer,
                    teacher_episode_carry,
                    teacher_reward,
                    done,
            ...
            *body, rng = runner_state
            ...
            # jax.debug.print("teacher_ep_step: {teacher_ep_step}", teacher_ep_step=teacher_ep_step)
            ...
            def _run_teacher_update(operands):
                jax.debug.print("updaing teacher")
                t_train_state, t_buffer, t_rng, t_carry = operands
                traj = gather_teacher_traj(t_buffer, teacher_rollout_buffer_size)
                last_val = t_carry.value
                advantages, targets = _teacher_calculate_gae(traj, last_val)
                flat_batch = flatten_teacher_traj(traj)
                flat_advantages = advantages.reshape((teacher_batch_size,))
                flat_targets = targets.reshape((teacher_batch_size,))
            ...
            def _skip_teacher_update(operands):
                t_train_state, t_buffer, t_rng, _t_carry = operands
                z = jnp.array(0.0, dtype=jnp.float32)
                jax.debug.print("skipping teacher update")
        ...
        final_teacher_rollout_buffer = (
            runner_state[-5] if enable_train_render else runner_state[-4]
        )
        train_output = {
