    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "u_maze" in self.maze_layout_name:
            ...
            self.min_x = jnp.array([1.0,1.0,TARGET_Z_COORD])
        if "big_maze" in self.maze_layout_name:
            self.max_x = jnp.array([14,14,TARGET_Z_COORD])
            self.min_x = jnp.array([1.,1,TARGET_Z_COORD])
        if "hardest_maze" in self.maze_layout_name:
            self.max_x = jnp.array([22,22,TARGET_Z_COORD])
            self.min_x = jnp.array([1.,1.,TARGET_Z_COORD])
