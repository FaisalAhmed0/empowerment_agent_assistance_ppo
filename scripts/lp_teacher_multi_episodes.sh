...
WANDB_PROJECT_NAME="mlp_teacher_lp_reward_multi_episodes_teacher_2"
...
TEACHER_LRS=(0.0003)
SEEDS=(30 75937 123)
COMMENT="multi_episodes_teacher"
...
TEACHER_ROLLOUT_BUFFER_SIZES=(1 5 10 64)
ABSOLUTE_LEARNING_PROGRESSS=(--no-ABSOLUTE_LEARNING_PROGRESS)
NUM_EVAL_ENVSS=(8)
TEACHER_ENTROPY_COEFSS=(0.05 0.01 0.005 0.001)
...
TEACHER_EPISODE_LENGTHS=(100 500 1000)
...
for ENV_NAME in "${ENV_NAMES[@]}"; do
  for TOTAL_TIMESTEPS in "${TOTAL_TIMESTEPS_[@]}"; do
    for LR in "${LRS[@]}"; do
                      ...
                      CMD="sbatch scripts/submit_job purejaxrl/ppo_continuous_action_custom_brax_with_teacher_simple_reward_dev.py \
                        --ENV_NAME=${ENV_NAME} \
