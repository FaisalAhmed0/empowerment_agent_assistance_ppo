...
WANDB_PROJECT_NAME="purejaxrl_all_envs_uniform_teacher_baseline"

ENV_NAMES=("ant_u_maze_single_goal" "ant_big_maze_single_goal")
SAVE_MODEL="--SAVE_MODEL"
TOTAL_TIMESTEPS_=(350000000)
LRS=(0.0003)
SEEDS=(30 75937 123 1 842434353)
COMMENT="uniform_teacher_baseline"
  ...
  for TOTAL_TIMESTEPS in "${TOTAL_TIMESTEPS_[@]}"; do
    for LR in "${LRS[@]}"; do
                    for SEED in "${SEEDS[@]}"; do
                      ...
                      CMD="sbatch scripts/submit_job purejaxrl/ppo_continuous_action_custom_brax_with_uniform_teacher.py \
                        --ENV_NAME=${ENV_NAME} \
