...
WANDB_PROJECT_NAME="discrete_actions_teacher_single_student_reward_weightings_5"
...
TEACHER_BATCH_SIZES=(64 1024 2048)
TEACHER_LP_EVAL_EVERY_UPDATES=(1 8 16)
GOAL_REWARD_WEIGHTS=(1)
USE_SEPARATE_VALUE_FUNCTIONS=("--USE_SEPARATE_STUDENT_VALUE_FUNCTIONS")
USE_ACTOR_PROBING_STATES=("--no-USE_ACTOR_PROBING_STATES")
USE_CRITIC_PROBING_STATES=("--no-USE_CRITIC_PROBING_STATES")
                  ...
                  for teacher_sample_every_n_episodes in "${TEACHER_SAMPLE_EVERY_N_EPISODES[@]}"; do
                    for SEED in "${SEEDS[@]}"; do
                      for teacher_num_minibatches in "${TEACHER_NUM_MINIBATCHES_[@]}"; do
                        ...
                        for teacher_lp_eval_every_updates in "${TEACHER_LP_EVAL_EVERY_UPDATES[@]}"; do
                      ...
                      CMD="sbatch scripts/submit_job purejaxrl/ppo_continuous_action_custom_brax_with_teacher_discrete.py \
                        ...
                        --TEACHER_LP_EVAL_EVERY_UPDATES=${teacher_lp_eval_every_updates} \
                        ${layer_norm} \
                        --NUM_STEPS=${num_steps} \
                        --CLIP_EPS=${clip_eps} \
