...
WANDB_PROJECT_NAME="discrete_actions_teacher_single_student_reward_weightings_3"
...
NUM_STEPS_=(64)
TEACHER_BATCH_SIZES=(16 64 1024)
...
LAYER_NORM=("--no-LAYER_NORM")
TASK_REWARD_WEIGHTS=(1)
GOAL_REWARD_WEIGHTS=(1)
                  ...
                  for teacher_sample_every_n_episodes in "${TEACHER_SAMPLE_EVERY_N_EPISODES[@]}"; do
                    for SEED in "${SEEDS[@]}"; do
                      for teacher_num_minibatches in "${TEACHER_NUM_MINIBATCHES_[@]}"; do
                        ...
                        for task_reward_weight in "${TASK_REWARD_WEIGHTS[@]}"; do
                        for goal_reward_weight in "${GOAL_REWARD_WEIGHTS[@]}"; do
                        for teacher_lp_absolute in "${TEACHER_LP_ABSOLUTES[@]}"; do
                        for teacher_batch_size in "${TEACHER_BATCH_SIZES[@]}"; do
                      RUN_NAME="${ENV_NAME}_steps${TOTAL_TIMESTEPS}_lr${LR}_teacherlr${teacher_lr}_probeagg${teacher_probe_agg}_treward${teacher_reward_type}_sgoal${student_goal_reward_type}_batchsize${teacher_batch_size}_sampleevery${teacher_sample_every_n_episodes}"
                      CMD="sbatch scripts/submit_job purejaxrl/ppo_continuous_action_custom_brax_with_teacher_discrete_2.py \
                        --ENV_NAME=${ENV_NAME} \
